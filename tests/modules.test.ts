...
import { app, prisma, getAuthToken, authGet, authPost } from "./setup.js";
...
describe("Module Endpoints", () => {
  ...
  describe("GET /api/modules/:moduleId/theme", () => {
    ...
    it("should reject request without auth token", async () => {
      const response = await request(app).get("/api/modules/1/theme");
  ...
  describe("GET /api/modules/overview", () => {
    ...
    it("should reject request without auth token", async () => {
      const response = await request(app).get("/api/modules/overview");
